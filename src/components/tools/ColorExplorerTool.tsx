...
export const ColorExplorerTool: React.FC<{ navigate: (route: string) => void }> = ({ navigate }) => {
  ...
  return (
    <div className="min-h-screen py-8">
      <SEO
        config={{
          title: `Smart Color Explorer & Picker — ${colorData.name} (${colorData.hex.toUpperCase()})`,
          description: `Analyze ${colorData.name} (${colorData.hex}). Instant conversion between HEX, RGB, HSL, HSV, CMYK, OKLCH, Tailwind shades 50-950, and harmonized palettes.`,
          canonicalUrl: `https://chromora.app/color-picker`,
          faqs: explorerFaqs,
          softwareApp: {
            name: 'Chromora Color Explorer',
            description: 'Intelligent color discovery and multi-space conversion tool.',
